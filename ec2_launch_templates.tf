...
data "aws_ami" "amazon2_minimal_latest_focal" {
  most_recent = true
  owners      = ["self", "amazon"]
  filter {
    name   = "name"
...
resource "aws_launch_template" "free_tier_amazon2_latest_focal" {
  name          = "FreeTierAmazon2LaunchTemplate"
  instance_type = var.free_tier_instance_type
  image_id      = data.aws_ami.amazon2_minimal_latest_focal.id
  network_interfaces {
